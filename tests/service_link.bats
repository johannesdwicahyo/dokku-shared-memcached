...
@test "service_link sets MEMCACHED_URL and MEMCACHED_KEY_PREFIX on the app" {
  service_link "demo" "myapp"
  assert_stub_called_with dokku "config:set --no-restart myapp MEMCACHED_URL=dokku-shared-memcached:11211 MEMCACHED_KEY_PREFIX=demo:"
}
...
@test "service_unlink unsets both vars and drops the LINKS entry" {
  printf 'myapp\nother\n' >"$PLUGIN_DATA_ROOT/demo/LINKS"
  service_unlink "demo" "myapp"
  assert_stub_called_with dokku "config:unset --no-restart myapp MEMCACHED_URL MEMCACHED_KEY_PREFIX"
  run cat "$PLUGIN_DATA_ROOT/demo/LINKS"
  [[ "$output" == "other" ]]
}
